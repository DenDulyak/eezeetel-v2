...
<body>
	...
	<%
		String strQuery = "from TMasterCustomerinfo where Customer_Group_ID = " + nCustomerGroupID + " order by Active_Status desc, Customer_Company_Name";
		...
		try {
			...
			for (int nIndex = 0; nIndex < records.size(); nIndex++)
			{
				...
				strCustomerDoingTransactionsColor = (custInfo.getActive()) ? strCustomerDoingTransactionsColor : strBgColor;
	...
	<tr bgcolor="<%=strBgColor%>">
			<td align="right">
				<input type="radio" name="record_id" value="<%=custInfo.getId()%>">
			</td>
			<td align="left" bgColor="<%=strCustomerDoingTransactionsColor%>">
				<a href="<%=ksContext%>/admin/Customers/ModifyCustomerInfo.jsp?record_id=<%=custInfo.getId()%>"> <%=custInfo.getCompanyName()%> </a>
			</td>
			<td align="left"> <font color=red> <%=custInfo.getCustomerBalance()%> </font></td>			
			<td align="left"> <%=custType.getCustomerType()%> </td>
			<td align="left"> <%=custInfo.getFirstName()%> </td>
			<td align="left"> <%=custInfo.getLastName()%> </td>
			<td align="left"> <%=custInfo.getMiddleName()%> </td>
			<td align="left"> <%=custInfo.getAddressLine1()%> </td>
			<td align="left"> <%=custInfo.getAddressLine2()%> </td>
			<td align="left"> <%=custInfo.getAddressLine3()%> </td>
			<td align="left"> <%=custInfo.getCity()%> </td>
			<td align="left"> <%=custInfo.getState()%> </td>
			<td align="left"> <%=custInfo.getPostalCode()%> </td>
			<td align="left"> <%=custInfo.getCountry()%> </td>
			<td align="left"> <%=custInfo.getPrimaryPhone()%> </td>
			<td align="left"> <%=custInfo.getSecondaryPhone()%> </td>
			<td align="left"> <%=custInfo.getMobilePhone()%> </td>
			<td align="left"> <%=custInfo.getWebsiteAddress()%> </td>
			<td align="left"> <%=custInfo.getEmailId()%> </td>
			<td align="left"> <%=userCreated.getUserFirstName()%> </td>
			<td align="left"> <%=userIntroduced.getUserFirstName()%> </td>
			<td align="left"> <%=custInfo.getLastModifiedTime()%> </td>
			<td align="left"> <%=custInfo.getCreationTime()%> </td>
			<td align="left"> <%=custInfo.getNotes()%> </td>
			<td align="left"> <%=strIsActive%> </td>
			<td align="left"> <%=strCustomerFeatues%> </td>
		</tr>
	<%
			}
		} catch(Exception e) {
	...
	%>
		...
		<tr>
			...
			<td align="center">
				<input type="button" name="add_button" value="Add" OnClick="SubmitForm('<%=ksContext%>/admin/Customers/NewCustomerInfo.jsp', false);">
			</td>
	
			<td align="center">
				<input type="button" name="modify_button" value="Modify" OnClick="SubmitForm('<%=ksContext%>/admin/Customers/ModifyCustomerInfo.jsp', true);">
			</td>
	
			<td align="center">
				<input type="button" name="activate_button" value="Activate" OnClick="SubmitForm('<%=ksContext%>/admin/Customers/ActivateCustomerInfo.jsp', true);">
			</td>
	
			<td align="center">
				<input type="button" name="delete_button" value="Delete" OnClick="SubmitForm('<%=ksContext%>/admin/Customers/DeleteCustomerInfo.jsp', true);">
			</td>
		</tr>
	</table>
	</form>
