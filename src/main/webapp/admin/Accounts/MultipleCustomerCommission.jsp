    ...
    <script language="javascript">
        ...
        function statusCheck(do_real_check) {
            ...
            for (var key in product_list) {
                ...
                if (Number(theObj.modified_group_commission) != Number(theObj.group_commission))
                    changed = true;
        ...
        function commission_changed(type, element, id) {
            ...
            if (type == 'ee') {
                ...
                if (Number(theObj.modified_agent_commission) > Number(element.value)) {
                    if (!confirm("Agent Commission is more than <%=strCustomerGroupName%> Commission.  Is this Correct?")) {
                        element.value = theObj.modified_group_commission;
                        element.focus();
                        return;
                    }
                }

                theObj.modified_group_commission = element.value;
                if (Number(theObj.modified_group_commission) != Number(theObj.group_commission))
                ...
                if (Number(theObj.modified_group_commission) < Number(element.value)) {
                    if (!confirm("Agent Commission is more than <%=strCustomerGroupName%> Commission.  Is this Correct?")) {
            ...
            var the_value = eval(theObj.unit_price) + eval(theObj.modified_group_commission);
            ...
            the_value = eval(theObj.unit_price) + eval(theObj.modified_group_commission) + eval(theObj.modified_agent_commission);
            the_element.value = the_value.toFixed(2);
            if (Number(the_element.value) >= Number(theObj.face_value))
                the_element.style.background = "#FF0000";
            else
            ...
            httpObj.onreadystatechange = function () {
                if (httpObj.readyState == 4) {
                    ...
                    if (document.the_form.supplier_id.value == transferToSupplier_ID || document.the_form.supplier_id.value == dingSupplierID)
                        theHtml += "<center><H1><font color=\"red\">Mobile Topup Profit Margins are on Percentage Basis</font></H1></center><BR><BR>";
                    theHtml += "<table border=1 width=100%>";
                    theHtml += ("<tr bgcolor=FFFF00><td align=center><b>Product Name</b></td><td align=center><b>Unit Price</b></td><td align=center><b><%=strCustomerGroupName%> Commission</b></td>");
                    theHtml += ("<td align=center><b>Agent Commission</b></td><td align=center><b>Notes</b></td>");
                    theHtml += ("<td align=center><b>Price After <%=strCustomerGroupName%> Commission</b></td><td align=center><b>Price After Agent Commission</b></td></tr>");
                    ...
                    for (var i = 0; i < nl.length; i++) {
                        ...
                        var group_commission = nli.getAttribute('group_commission');
                        var agent_commission = nli.getAttribute('agent_commission');
                        var notes = nli.getAttribute('notes');
                        var active = nli.getAttribute('active');
                        var price_after_group_commission = eval(unit_price) + eval(group_commission);
                        var price_after_agent_commission = eval(price_after_group_commission) + eval(agent_commission);

                        theHtml += ("<tr>");
                        theHtml += ("<td>" + name + " - " + face_value + "</td>");
                        theHtml += ("<td>" + unit_price + "</td>");
                        theHtml += ("<td align=center><input type=text size=4 maxlength=4 name=ee_" + id + " value=\"" + group_commission + "\" onChange=\"commission_changed('ee', this, " + id + ")\"></td>");
                        ...
                        theHtml += ("<td align=center><input type=text size=4 readonly id=pae_" + id + " value=\"" + price_after_group_commission.toFixed(2) + "\"></td>");
                        ...
                        prodObj.group_commission = group_commission;
                        prodObj.agent_commission = agent_commission;
                        prodObj.notes = notes;
                        prodObj.active = active;
                        prodObj.unit_price = unit_price;
                        prodObj.modified_group_commission = group_commission;
        ...
        function makeUpChangedProductsList() {
            ...
            for (var key in product_list) {
                ...
                if (Number(theObj.modified_group_commission) != Number(theObj.group_commission) ||
                    ...
                    changed_objects += (" ee_comm=\"" + theObj.modified_group_commission) + "\"";
        ...
        function SaveInformation() {
            ...
            var url = "/admin/Accounts/AJAX_AddMultipleCustomerCommission.jsp?" +
        ...
        function CopyToCustomer() {
            ...
            if (!document.the_form.copy_agent_only.checked) {
                if (!confirm("Are you sure, you want to copy the <%=strCustomerGroupName%> commission also to this customer."))
                    return;
            }
            ...
            var url = "/admin/Accounts/AJAX_CopyCustomerCommission.jsp?to_customer_id=" + to_cust_id +
                ...
                <select name="agent_id" id="the_agent_id" onchange="populateCustomers()">
                    ...
                    <%
                        ...
                        try {
                            ...
                            String strQuery = "from User where Customer_Group_ID = " + nCustomerGroupID +
                                    " and User_Type_And_Privilege in (4,6) and User_Login_ID != 'Pending' and User_Active_Status = 1 order by User_First_Name";
