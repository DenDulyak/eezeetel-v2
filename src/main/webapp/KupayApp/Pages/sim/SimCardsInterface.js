function list_sim_products(supplier_id)
{
	...
	var url = theContext + "/Pages/sim/AJAX_GetSIMCardProductsInfo.jsp?supplier_id=" + supplier_id;
...
function do_sim_transaction(product_id)
{
	...
	document.the_form.action = theContext + "/Pages/sim/CheckSelectedSimProducts.jsp?sim_id=" + requiredProduct;
...
{
	document.the_form.reset();
	document.the_form.action = theContext + "/Pages/ShowProducts.jsp";
	document.the_form.submit();
}
...
function confirm_sim_transaction()
{
	...
	/*if (IsNULL(document.the_form.mob_topup_1.value))
	{
		alert("Please select at least one mobile topup transaction to allot the SIM.");
		return;
	}*/

	document.the_form.action = theContext + "/Pages/sim/CompleteSimTransaction.jsp?";
	document.the_form.submit();	
}